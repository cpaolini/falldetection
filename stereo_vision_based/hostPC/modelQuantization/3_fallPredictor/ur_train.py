...
class loadDataset(Dataset):
    ...
    def read_data_from_json(self, json_file_path, window_size):
        ...
        for group_id, data in grouped_data_dict.items():
            keypoints = torch.Tensor(data['keypointsWorld'])
            label = data['isFall']
...
def train_model(train_dataloader, test_dataloader, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if accuracy_val_add > prev_accuracy:
            torch.save(model.state_dict(), "/mnt/beegfs/home/ramesh/Quantization/FallDetector/build/float_model/ur_fall/"+str(epoch)+"_fall_detection_model.pth")
...
def main():
    ...
    test_dataset = loadDataset('/mnt/beegfs/home/ramesh/Quantization/FallDetector/grouped_data_test.json', window_size = 5)
