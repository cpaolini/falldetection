...
if DATASET == 'ITOP':
    ...
    trainingImageDir = ''
    testingImageDir = './depth_maps_100/'
    Img_mean = 3
    Img_std = 2
    imgWidth = 480
    imgHeight = 512
    
    with open('./labels100.json') as f:
        dict = json.load(f)
    ...
    trainingImageDir = ''
    testingImageDir = './validDepthTestMAT/'
    ...
    with open('./UR_fall_valid_test.json') as f:
        dict = json.load(f)
        annotations_test = dict
    ...
    trainingImageDir = ''
    testingImageDir = './sdsu_psg_depth_validationSet/'
    ...
    with open('./SDSU_PSG_a2j_test.json') as f:
        dict = json.load(f)
        annotations_test = dict
...
def evaluation2D_perJoint(source, target, dist_th_2d):
  ...
  for j in range(keypointsNumber):
    for i in range(len(source)):
      if np.square(source[i, j, 0] - target[i, j, 0]) + np.square(source[i, j, 1] - target[i, j, 1]) < np.square(dist_th_2d):
        count = count + 1
...
def dataPreprocess(index, imgDir, keypointsWorld, keypointsPixel, bndbox, mode, augment=True):
  if DATASET == 'UR_Falldetection':        
      ...
      img = np.load(imgDir + str(index) + '.npy').astype(float)
  elif DATASET == 'MP3DHP':
        img = np.load(os.path.join(imgDir, test_image_ids[index]), allow_pickle=True).astype(float)
  elif DATASET == 'ITOP':
  ...
  # image = img.reshape(480, 640, 1)
  # cv2.imwrite('/mnt/beegfs/home/ramesh/Quantization/A2J/tempDir/' + str(index) + '_img.png',image)
  # print(f"image #{index}\timg_shape = {img.shape}\tfilename = {test_image_ids[index]}\tbndbox = {bndbox[index][0], bndbox[index][1], bndbox[index][2], bndbox[index][3]}")
  ...
  # im = Image.fromarray(np.uint8(imCrop * 255), 'L')
  # I1 = ImageDraw.Draw(im)
  # I1.text((28, 36), str(index))
...
class my_dataloader(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    data, label, orig_img, index = dataPreprocess(index, self.ImgDir, self.keypointsWorld, self.keypointsPixel, self.bndbox, self.mode, self.augment)
    if self.augment:
...
def test(qmodel, device, test_dataloaders):
  # qmodel = qmodel.to(device)
  qmodel = qmodel.eval()
  
  post_precess = anchor.post_process(shape=[cropHeight//16,cropWidth//16],stride=16,P_h=None, P_w=None)
  # post_precess = anchor.post_process_cpu(shape=[cropHeight//16,cropWidth//16],stride=16,P_h=None, P_w=None)
  ...
  for i, (img, label, orig_img, index) in tqdm(enumerate(test_dataloaders)):    
    with torch.no_grad():       
      ...
      if saveFrame == True:
        ...
        single_img = single_img.astype(np.uint8)
        cv2.imwrite(destPath + 'output_' + str(i) + '.png', single_img)
...
def quantize(build_dir,quant_mode,batchsize):
  ...
  if (torch.cuda.device_count() > 0):
    ...
    # print('Selecting device 0 1 2 3')
  ...
  # net.load_state_dict(torch.load(os.path.join(float_model,'URFall_epoch16.pth')))
  net.load_state_dict(torch.load(os.path.join(float_model,'net_15_wetD_0.0001_depFact_1_RegFact_5_rndShft_15.pth')))
  net = net.eval()
  logger.info("loading the flaot model completed. :)")
  print(f"Device active = {device}")

  if (quant_mode=='inspect'):
    inspector = Inspector("DPUCZDX8G_ISA1_B4096") 
    input_shape=([batchsize, 1, 288, 288])
    test_input = torch.rand(input_shape).to(device)
    inspector.inspect(net.module, test_input, device=device)
    sys.exit()

  '''
  jit trace test
  '''
  #  # input_shape=([64, 1, 288, 288])
  # test_input = torch.rand(input_shape).to(device)
  # try:
  #     tr_func = torch.jit.trace(net.module, test_input)
  ...
  rand_in = torch.randn([batchsize, 1, 288, 288])
  quantizer = torch_quantizer(quant_mode, net.module, (rand_in), output_dir=quant_model) 
  ...
  test_dataloaders = torch.utils.data.DataLoader(test_image_datasets, batch_size = batchsize,
  shuffle = False, num_workers = 8)
  
  if quant_mode == 'calib':
    quantizer.fast_finetune(test, (quantized_model, device, test_dataloaders))
  elif quant_mode == 'test':
    quantizer.load_ft_param()

  # evaluate the quantized_model
  test(quantized_model, device, test_dataloaders)
  # test(net, device, test_dataloaders)
...
def run_main():
  ...
  ap.add_argument('-q',  '--quant_mode', type=str, default='inspect',    choices=['calib','test', 'inspect'], help='Quantization mode (calib or test). Default is inspect')
