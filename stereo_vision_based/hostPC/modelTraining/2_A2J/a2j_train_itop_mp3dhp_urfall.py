...
if DATASET == 'ITOP':
    ...
    imgWidth = 480
    imgHeight = 640
    save_dir = './result/SDSU_PSG/'
    annotations_train = dict()

    with open(trainingImageDir) as f:
...
def dataPreprocess(index, imgDir, keypointsWorld, keypointsPixel, bndbox, mode, augment=True):
    if DATASET == 'UR_Falldetection':        
        ...
        if mode == 'TEST':
            img = np.load(test_dict[test_image_ids[index]]).astype(float)
        else:
            img = np.load(train_dict[train_image_ids[index]]).astype(float)
    elif DATASET == 'MP3DHP':
        if mode == 'TEST':
...
#           visualize input images to the network with GT points -- block taken form MP3DHP
            ...
            # plt.show()
            single_img = single_img.astype(np.uint8)
            cv2.imwrite(save_dir + '/images/trainOut/' + str(i) + '.png', single_img)
