...
train_image_datasets = my_dataloader(trainingImageDir, keypointsWorldtrain, keypointsPixeltrain, TrainImgFrames, bndbox_train, mode = 'TRAIN', augment=True)
...
#                   Visualize outputs in Validation - block taken from MP3DHP
                    ...
                    plt.imshow(single_img)
                    single_img = single_img.astype(np.uint8)
                    cv2.imwrite(save_dir + '/images/output/epoch_' + str(epoch) + '_output_' + str(i) + '.png', single_img)
#    
        ...
        %(epoch, train_loss_add, Cls_loss_add, Reg_loss_add, Error_test, scheduler.get_last_lr()[0], train_loss_add, val_loss_add, Accuracy_test))
