...
testingImageDir = '/mnt/beegfs/home/ramesh/Datasets/MP-3DHP/ardata/dataset/depthMaps/'
# testingImageDir = '/mnt/beegfs/home/ramesh/Datasets/MP-3DHP/ardata/dataset/train_val/depth_maps/'

model_dir = '/mnt/beegfs/home/ramesh/A2J/src_train/result/MP3DHP_batch_64/net_34_wetD_0.0001_depFact_0.5_RegFact_3_rndShft_5.pth'
...
with open('/mnt/beegfs/home/ramesh/Datasets/MP-3DHP/ardata/labels/labelMap/labels.json') as f:
...
def main():
    ...
    net.load_state_dict(torch.load(model_dir), strict=True)
    # net = net.cuda()
    # net = torch.nn.DataParallel(net).to(device)
    net.eval()
    ...
    for i, (img, label, orig_img, index) in tqdm(enumerate(test_dataloaders)):
        with torch.no_grad():
            ...
            #             # print(f"\nShape of single_img = {single_img.shape}")
            #             # print(f"\nShape of orig_img_ = {orig_img_.shape}")
            #             # print(f"\nk = {k}\nj = {j}")
            #             # print(f"\nsingle_bndbox[0] = {single_bndbox[0]}")
            #             # print(f"\nsingle_bndbox[1] = {single_bndbox[1]}")
            #             # print(f"\nsingle_bndbox[2] = {single_bndbox[2]}")
            #             # print(f"\nsingle_bndbox[3] = {single_bndbox[3]}")
            #             # plt.imshow(single_img)
            #             reconstructed_img[k][j] = single_img[k - int(single_bndbox[1])][j - int(single_bndbox[0])][2]
            #         else:
    ...
    for i in tqdm(range(len(test_image_ids) - 1)):
        single_img = np.load(os.path.join(testingImageDir, test_image_ids[i])).astype(float)
        depth_max = 6
        single_img[single_img >= depth_max] = depth_max
        single_img /= depth_max
        single_img *= 255
        ...
        single_img = draw_humans_visibility(single_img,
                                           ...
                                           kp_connections(get_keypoints()),
                                           jointColors)
        cv2.imwrite(save_dir + '/images/padding_test/testing_'+ str(i) + '.png', single_img)
    ...
    TestWorld[:, :, 2] = Test1[:, :, 2]
    np.save('/mnt/beegfs/home/ramesh/A2J/bgaug_result_gtbox/result/TestWorld.npy', TestWorld)
    TestWorld = np.load('/mnt/beegfs/home/ramesh/A2J/bgaug_result_gtbox/result/TestWorld.npy')
...
def evaluation10CMRule_perJoint(source, target, Bndbox):
    ...
    accuracy = 0
    print("-------------------------------------------------------------------------------------------")
    print(f"TestWorld = {source[0]}")
    print("-------------------------------------------------------------------------------------------")
    print(f"keypointsUVD_test = {target[0]}")
    print("-------------------------------------------------------------------------------------------")
    for j in range(keypointsNumber):
        for i in range(len(source)):
            if np.square(source[i, j, 0] - target[i, j, 0]) + np.square(
