...
# import fallDetector_app_1024 as fallDetector
...
Fall Detector
'''
MAX_POSE_JOINTS = 200  # Maximum number of poseJoints to store
...
def runYolox(image, dpu_runner, saveVideo):
    global ratio , count_a2j, count_yolox, frame_count
...
def runPoseEstimation(image, Bbox, dpu_runner, saveVideo):
    global count_a2j, count_yolox, frame_count
    ''' preprocess images '''
...
def a2j_thread():   
    global numberOfBoxes, count_a2j, stop_a2j_event
    
    while not stop_a2j_event.is_set():
        try:
...
def main():
    global frame_count, count_a2j, count_yolox, saveVideo, pose_joints_list, numberOfBoxes, boundaryBox, OTF, stop_a2j_event
